...
func NewKeyFromMnemonic(mnemonic string, coin, account, chain, address uint32) (*bip32.ExtendedKey, error) {
...
func NewKeyFromMasterKey(masterKey *bip32.ExtendedKey, coin, account, chain, address uint32) (*bip32.ExtendedKey, error) {
	child, err := masterKey.Child(Purpose)
	if err != nil {
		return nil, err
	}

	child, err = child.Child(coin)
	if err != nil {
		return nil, err
	}

	child, err = child.Child(account)
	if err != nil {
		return nil, err
	}

	child, err = child.Child(chain)
	if err != nil {
		return nil, err
	}

	child, err = child.Child(address)
	if err != nil {
		return nil, err
	}
